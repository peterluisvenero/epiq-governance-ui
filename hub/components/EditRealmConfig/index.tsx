...
export function EditRealmConfig(props: Props) {
  ...
  const govData = useGovernanceByPubkeyQuery(realm?.account.authority).data?.result;
    ...
    if (realm) {
      ...
      fetchConfig(
        ...
        .then((config) => {
          setConfig({ ...config });
          setProposalTitle(
            `Update Realms Config for "${realm.account.name}"`,
          );
  ...
  useEffect(() => {
    if (govData && realm) {
      ...
      let updateGovernance: Governance = {
        version: version ?? 3,
        walletAddress,
        ...data,
        communityTokenRules: {
          ...
          totalSupply: communityMint ? 
            new BigNumber(communityMint.supply.toString()).shiftedBy(-communityMint.decimals) : 
            new BigNumber(0),
          tokenMintDecimals: communityMint ? new BigNumber(communityMint.decimals) : new BigNumber(0),
        },
        councilTokenRules: councilMint && data.councilTokenRules ? {
          ...data.councilTokenRules,
          canCreateProposal: data.councilTokenRules.canCreateProposal!, 
          totalSupply: councilMint ? 
            new BigNumber(councilMint.supply.toString()).shiftedBy(-councilMint.decimals) : 
            new BigNumber(0),
          tokenMintDecimals: communityMint ? new BigNumber(communityMint.decimals) : new BigNumber(0),
        } : null
      }
